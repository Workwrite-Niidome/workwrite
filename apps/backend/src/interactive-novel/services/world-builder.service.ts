...
export class WorldBuilderService {
  ...
  async generateExperienceScript(
    ...
    const ppResult = postProcessExperienceScript(allScenes, introData, sourceEpisodes.length, characters);
    onProgress?.(`接続修復: ${ppResult.fixedConnections}, 参照修復: ${ppResult.fixedRefs}, 行き止まり修復: ${ppResult.fixedDeadEnds}, 話者名統一: ${ppResult.normalizedSpeakers}`);
    this.logger.log(`Post-process: connections=${ppResult.fixedConnections}, refs=${ppResult.fixedRefs}, deadEnds=${ppResult.fixedDeadEnds}, speakers=${ppResult.normalizedSpeakers}`);
