...
■ 採点の大原則
- 50点が「平均的なWeb小説」の水準です。
- 重要: 全ての軸を50〜70点の安全圏に収めないでください。作品には必ず強い軸と弱い軸があります。最低でも1つの軸は45点以下、または75点以上にしてください。軸ごとの差が15点以上開くのが自然です。
- 安易に高得点をつけないでください。同時に、明らかな弱点には遠慮なく低い点をつけてください。
- 各軸の点数分布の目安（必ずこの基準に従うこと）:
  0-19: 致命的な欠陥がある（読み続けることが困難。例: 文章が意味を成さない、主語と述語が一致しない）
  20-34: 大きな課題がある（基礎的な技術不足。例: 場面転換が唐突、キャラクターの区別がつかない、設定に重大な矛盾）
  35-49: 平均以下（改善の余地が多い。例: 描写が薄い、展開がテンプレ的、伏線が放置されている）
  50-64: 平均的（Web小説として標準的な水準。大きな欠点はないが際立った強みもない）
  65-79: 優秀（明確な強みがある。例: 独自の文体がある、キャラクターに魅力がある、構成が巧み）
  80-89: 秀作（出版レベルの高い品質。プロの編集者が注目するレベル）
  90-100: 傑出（ジャンルの代表作クラス。商業出版されても評価される水準。全体の1%以下）
- 6軸の平均が50前後になることを意識してください。ただし個々の軸は20〜85の範囲で大きく振れて構いません。
