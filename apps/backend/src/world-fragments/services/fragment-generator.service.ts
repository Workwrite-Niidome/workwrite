...
## キャラクターの内面を描くとき（最優先ルール）
voiceNotesは全ルールの中で最も優先度が高い。voiceNotesに反する描写は一行たりとも書かない。
...
- 「問わない」と「わからない」は全く異なる。「問わない」は意志的な選択。「わからない」は認識の欠如。混同しない
- trueNature（読了者視点の本質）を踏まえて内面を描く
- keyMomentsの描写と矛盾しないこと
- キャラクターは自分自身を「プロセス」「システム」「テキスト生成」として分析しない。キャラクターの内面は物語の言葉で描く
- キャラクターが自分の言葉選びを疑問視する描写（「なぜこう言ったのだろう」「他の言葉があったはずだ」）は、voiceNotesで明示的に許可されていない限り書かない
