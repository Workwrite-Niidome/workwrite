...
function FeaturesSection() {
  const features = [
    ...
    },
    {
      ...
      description: '読後の感動をギフトレターとして著者に送れます。ショート（¥120）からギフト（¥1,000〜）まで4タイプ。収益の80%が著者に還元されます。',
    },
    {
      icon: Share2,
      title: 'SNSシェア＆OGPカード',
      ...
      <div className="mx-auto max-w-4xl px-4 md:px-6 py-20 sm:py-28">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              読後の感動をギフトレターとして著者に送れます。スタンプ付きのメッセージで、あなたの「ありがとう」を形に。
            </p>
          </div>
          <div className="text-center">
            <div className="w-12 h-12 rounded-full bg-secondary flex items-center justify-center mx-auto mb-4">
...
function FaqSection() {
  const faqs = [
    {
      q: '利用料金はかかりますか？',
      a: '作品の執筆・公開・閲覧は無料です。AI執筆アシストは月20回まで無料でご利用いただけます。より多くのAI機能を使いたい方には、Standard（¥2,980/月）とPro（¥7,980/月）の有料プランをご用意しています。詳しくは料金プランをご覧ください。',
    },
    {
    ...
    {
      q: 'ギフトレター（投げ銭）の仕組みは？',
      a: '読者がエピソードを読んだ後、著者にギフトレターを送れます。ショート（¥120）、スタンダード（¥300）、プレミアム（¥500）、ギフト（¥1,000〜）の4タイプがあり、収益の80%が著者に還元されます。ギフトレター内容はAIが自動審査するため、誹謗中傷は届きません。',
    },
  ];
