...
### 感情タグ→発見フィードバックパイプライン（MVP）
```
読者が感情タグ投稿
  -> EmotionTag DB保存
    -> 作品の感情プロファイル更新（タグ×件数の集約ビュー / Materialized View）
      -> Meilisearch 感情タグファセット更新（データ同期ジョブ経由）
        -> 感情状態ベース探索の精度向上
        -> 「次の一冊」簡易ナビの候補精度向上
        -> 「埋もれた名作」セクションのタグ多様性向上
```

このパイプラインにより、読者の読後感が蓄積されるほど発見システムの精度が向上する好循環を実現する。
MVPでは Materialized View の定期リフレッシュ（15分間隔）で簡易実装し、Phase 2 でリアルタイム更新に移行する。
