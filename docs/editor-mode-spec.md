...
**UI:**
- 設計書の一覧表示（キャラクター、世界観、プロット、トーン、読後感、各話概要）
- 各セクションに2つの操作:
  - 「編集」— 人間が直接テキストを編集できるフォーム
    - キャラクター: 名前、性格、口調、役割、外見などを個別に編集
    - プロット: 各話の概要・順序を変更、話の追加・削除
    - 世界観: ルール、用語、設定のテキスト編集
    - トーン・読後感: 自由テキスト編集
  - 「AIに修正させる」— テキストで修正指示 → AIが書き直し（1cr or 5cr/回）
- 変更はリアルタイムでDBに保存（自動保存）
- 「設計を確定して第1話を生成」ボタン（人間が納得してから次へ進む）
